...
import {executeHiggsfieldGenjutsuAdapter} from "./higgsfield-adapter.js";
...
const server=http.createServer(async(req,res)=>{try{
 if(req.method==="GET"&&req.url==="/health")return json(res,200,{ok:true,service:"acs-v3-runtime",version:"0.3.0"});
 if(req.method==="POST"&&req.url==="/api/adapters/higgsfield"){const input=await readBody(req);const result=await executeHiggsfieldGenjutsuAdapter(input);return json(res,200,result);}\n if(req.method==="POST"&&req.url==="/api/projects"){const input=await readBody(req),result=await executeGoldenPath(input),db=await load();db[result.project.id]=result;await save(db);return json(res,201,result);}
